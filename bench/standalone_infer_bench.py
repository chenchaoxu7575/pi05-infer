...
def _read_clocks(device_index: int) -> dict:
    ...
    ``device_index`` is a torch ordinal into the *visible* devices; NVML indexes
    all physical devices and ignores ``CUDA_VISIBLE_DEVICES``, so indexing NVML
    with it reads the wrong card. Resolve by UUID, which means the same thing on
    both sides.
    """
    try:
        import pynvml
    ...
    try:
        ...
        try:
            ...
            # but refuse to report a possibly-wrong card's clock as this one's.
            h = pynvml.nvmlDeviceGetHandleByIndex(device_index)
            if pynvml.nvmlDeviceGetCount() > 1 and os.environ.get("CUDA_VISIBLE_DEVICES"):
                return {
