...
class PI0Pytorch(nn.Module):
    ...
    def embed_prefix(
        ...
        all_embs = self._apply_checkpoint(
        ...
        for view_idx, img_mask in enumerate(img_masks):
            img_emb = all_embs[view_idx * bsize : (view_idx + 1) * bsize]
            embs.append(img_emb)
            pad_masks.append(img_mask[:, None].expand(bsize, num_img_embs))

            # Create attention masks so that image tokens attend to each other
            att_masks += [0] * num_img_embs
        nvtx.pop_range()  # prefix/vision_siglip

        # Process language tokens
        nvtx.push_range("prefix/lang_embed", color="cyan")
        ...
        nvtx.pop_range()  # prefix/lang_embed
